...
集群的"同款元素同款样式"以 [POMASA-STYLE-INVENTORY.md](POMASA-STYLE-INVENTORY.md) 为唯一参考（从 pomasa-studio `styles.js` 逐条转录）：按钮/输入/导航行/阶段条/卡片/modal/正文排印等共用元素，应用侧一律复用其中数值，不另起炉灶。新增共用元素时先查该清单并补录。
